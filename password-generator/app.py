...
class PasswordGenerator(QMainWindow):
    ...
    def connect_slider_to_spinbox(self) -> None:
        self.ui.slider_length.valueChanged.connect(self.ui.spinBox.setValue)
        self.ui.spinBox.valueChanged.connect(self.ui.slider_length.setValue)
    ...
    def set_strength(self) -> None:
        ...
        for strength in password.StrengthToEntropy:
            if password.get_entropy(length, char_num) >= strength.value:
                self.ui.label_strength.setText(f'Стойкость: {strength.name}')
